...
var TIMER ={
    ...
    tick: function(){
        ...
        if (hooked){
            if(difficulty === 1 && TIMER.i % 4 === 0){
                PS.statusText( "Time: " + time + " || Score: " + score + " || Reel!" );
                run++;
            }
            if (difficulty === 2 && TIMER.i % 3 === 0){
                PS.statusText( "Time: " + time + " || Score: " + score + " || Reel!" );
                run++;
            }
            if (difficulty === 3 && TIMER.i % 2 === 0){
                PS.statusText( "Time: " + time + " || Score: " + score + " || Reel!" );
                run++;
            }
            ...
            if (run === 6){
                fishRun();
                PS.statusText( "Time: " + time + " || Score: " + score + " || It Ran Away!" );
            }
    ...
    startTimer: function(until, functionToDo){
        ...
        TIMER.timerReference = PS.timerStart(10, TIMER.tick);
    }
}
...
PS.enter = function( x, y, data, options ) {
    ...
    if (menu){
        if ( y === 3){
            if (x === 1){
                PS.statusText( "Easy: Slow Fish Movement + Hooked longer" );
            }
            if (x === 3){
                PS.statusText( "Medium: Average Fish Movement" );
            }
            if (x === 5){
                PS.statusText( "Hard: Fast Fish Movement + Hooked shorter" );

            }
        }

        if (y === 5 && x === 3){
            PS.statusText( "Controls: Click fish to hook, Scroll to reel" );
        }
    }
};
...
function fillBucket(){
    if (finalscore < score){
        ...
        dropping = false;
        if (finalscore > highscoreEasy && difficulty === 1) {
            highscoreEasy = finalscore;
            PS.statusText("New Easy High Score! : " + highscoreEasy + " || R to Replay");
            PS.audioPlay("fx_tada");
        }
        else if (finalscore > highscoreMedium && difficulty === 2) {
            highscoreMedium = finalscore;
            PS.statusText("New Easy High Score! : " + highscoreEasy + " || R to Replay");
            PS.audioPlay("fx_tada");
        }
        else if (finalscore > highscoreHard && difficulty === 3) {
...
function newGame(){
    ...
    TIMER.startTimer(360, endScreen);
}
...
function makeMenu(){
    ...
    PS.color(1, 3, PS.COLOR_GREEN);
    PS.color(3, 3, PS.COLOR_YELLOW);
    PS.color(5, 3, PS.COLOR_RED);

    PS.color(3, 5, PS.COLOR_GRAY_LIGHT);
